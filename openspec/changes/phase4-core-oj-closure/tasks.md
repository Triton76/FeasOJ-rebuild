...
- [ ] 1.3 Complete submission worker/consumer lifecycle to ensure `pending -> judging -> terminal` state flow
- [ ] 1.4 Add idempotent terminal writeback handling and invalid-transition conflict checks
- [ ] 1.5 Add structured logs and metrics for enqueue/dequeue/writeback success, failure, and latency
- [ ] 1.6 Add integration tests for submit enqueue, consume transition, writeback success, writeback replay, and auth failure

## 2. Testcase Authoring and Judge Binding

- [ ] 2.1 Add testcase domain contracts (DTOs/ports) and teacher-admin authorization rules
- [ ] 2.2 Implement testcase repository CRUD with deterministic ordering and sample flag support
- [ ] 2.3 Add testcase HTTP handlers/routes for create, list, update, delete, and reorder operations
- [ ] 2.4 Implement testcase payload validation (required fields, size limits, count limits)
- [ ] 2.5 Implement judge-facing testcase read endpoint/service for authoritative active testcase sets
- [ ] 2.6 Add unit/integration tests for permissions, validation, ordering, and judge-read behavior
...
- [ ] 5.1 Add class navigation entry and route structure for class management and membership workflows
- [ ] 5.2 Implement teacher class create/edit/archive pages and API integration
- [ ] 5.3 Implement student class join and membership status pages
- [ ] 5.4 Implement assistant/teacher membership review interface with role-aware action visibility
...
- [ ] 6.2 Prepare additive database migrations for testcase and submission metadata needs
